...
s3api() {
    aws s3api "$1" --region "$AWS_DB_BACKUPS_REGION" --bucket "$AWS_DB_BACKUPS_BUCKET_PROD" "${@:2}"
}

bucket_exists() {
    s3 ls "$AWS_DB_BACKUPS_BUCKET_PROD" &> /dev/null
}

create_bucket() {
    echo "Bucket $AWS_DB_BACKUPS_BUCKET_PROD doesn't exist. Creating it now..."
...
upload_to_bucket() {
    ...
    s3 cp - "s3://$AWS_DB_BACKUPS_BUCKET_PROD/$(date +%Y/%m/%d/backup-%H-%M-%S.sql.gz)"
}
